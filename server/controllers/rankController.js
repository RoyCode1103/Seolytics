...
export const addKeyword = async(req, res) => {
    try{
        ...
        const existing = await KeywordTracking.findOne({userId: req.userId, keyword: keyword.toLowerCase().trim(),
            domain});
            ...
            keywordTracking(tracking);
                ...
                if(error.code === 110000) return res.status(400).json({success: false,
...
// Get all tracked keywords for users
...
export const getKeyword = async(req, res) => {
    try{
        ...
        if(!tracking) return res.status(404).json({success: false, message: "Keyword tracking are not found"});
